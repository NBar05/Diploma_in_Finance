...
pool11 <- plm(g_spread_bp ~ illiquidity + dist_to_default + days_left + current_coupon + zero_coupon_bond, 
              index = c("bond", "date"), data = prep_week_data, model = "pooling")
pool12 <- plm(g_spread_bp ~ illiquidity + prob_of_default + days_left + current_coupon + zero_coupon_bond, 
              index = c("bond", "date"), data = prep_week_data, model = "pooling")

pcdtest(pool11, test = c("lm"))
pbgtest(pool11)

fixed11 <- plm(g_spread_bp ~ illiquidity + dist_to_default + days_left + current_coupon + zero_coupon_bond, 
               index = c("bond", "date"), data = prep_week_data, model = "within", effect = "twoways")
fixed12 <- plm(g_spread_bp ~ illiquidity + prob_of_default + days_left + current_coupon + zero_coupon_bond, 
               index = c("bond", "date"), data = prep_week_data, model = "within", effect = "twoways")
...
random11 <- plm(g_spread_bp ~ illiquidity + dist_to_default + days_left + current_coupon + zero_coupon_bond, 
                index = c("bond", "date"), data = prep_week_data, model = "random", effect = "twoways")
random12 <- plm(g_spread_bp ~ illiquidity + prob_of_default + days_left + current_coupon + zero_coupon_bond, 
                index = c("bond", "date"), data = prep_week_data, model = "random", effect = "twoways")
...
for (k in clusters$cluster) {
  ...
  random011 <- plm(g_spread_bp ~ illiquidity + dist_to_default + days_left + current_coupon + zero_coupon_bond,
                   index = c("bond", "date"), data = mini_data, model = "random", effect = "twoways")
  all_models[[i]] = random011
  i <- i + 1
  
  random012 <- plm(g_spread_bp ~ illiquidity + prob_of_default + days_left + current_coupon + zero_coupon_bond,
                   index = c("bond", "date"), data = mini_data, model = "random", effect = "twoways")
